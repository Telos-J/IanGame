window.addEventListener("load", function () {
    ...
    this.update = function () {
      ...
      switch (this.state) {
        case "follow":
          ...
          // if ((player.x <= (this.getRight() + world.tileWidth)) && (player.x >= (this.getLeft() - world.tileWidth)) && (player.y >= (this.getBottom() + world.tileHeight)) && (player.y <= (this.getTop() - world.TileHeight))) {
          //     console.log('pounceready');
          // }
          if (
  ...
  const Camera = function () {
    ...
    this.moveCamera = function (dx, dy) {
      if (
        player.x > world.map[0].length * world.tileWidth * 0.25 &&
        player.x < world.map[0].length * world.tileWidth * 0.75
      ) {
        this.x += dx;
      }
      if (
        player.y > world.map.length * world.tileHeight * 0.25 &&
        player.y < world.map.length * world.tileHeight * 0.6
      ) {
        this.y += dy;
      }
